...
describe('LarkOIDC2OAuthServerProvider', () => {
  ...
  it('returns a machine-readable reauth_required error when OIDC code exchange fails', async () => {
    const provider = new LarkOIDC2OAuthServerProvider(options);
    (authStore.getTransaction as jest.Mock).mockResolvedValue({
      txId: 'tx-1',
      clientId: 'test-client-id',
  ...
  it('returns a machine-readable reauth_required error when OIDC refresh fails', async () => {
    const provider = new LarkOIDC2OAuthServerProvider(options);
